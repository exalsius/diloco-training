...
class Wav2Vec2Output:
    ...
    def __init__(self, loss: Optional[torch.Tensor] = None, logits: torch.Tensor = None):
        self.loss = loss
        self.logits = logits
...
class Wav2Vec2Model(nn.Module):
    ...
    def forward(self, input_values: torch.Tensor, 
                labels: Optional[List[torch.Tensor]] = None) -> Union[torch.Tensor, Wav2Vec2Output]:
        """
        ...
        if labels is not None:
            ...
            if isinstance(labels[0], torch.Tensor):
                target_lengths = torch.tensor([label.size(0) for label in labels], 
                                             dtype=torch.long, 
                                             device=input_values.device)
            else:
...
def get_wav2vec2(hidden_size: int = 128,
                intermediate_size: int = 512,
                num_hidden_layers: int = 6,
                num_attention_heads: int = 4) -> Wav2Vec2Model:
    """
