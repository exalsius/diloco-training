...
class BigGANWithLoss(nn.Module):
    ...
    def __init__(
        self,
        model_type: str = "biggan-deep-256",
    ...
    def _initialize_model(self, model_type: str, pretrained: bool) -> None:
        ...
        if model_type == "biggan-deep-256":
            self.model = (
                ...
                if pretrained
                else BigGAN.from_pretrained("biggan-deep-256", pretrained=False)
            )
        else:
    ...
    ) -> Union[torch.Tensor, BigGANOutput]:
        """
...
def get_biggan(
    model_type: str = "biggan-deep-256", pretrained: bool = True
) -> BigGANWithLoss:
    """
...
if __name__ == "__main__":
    ...
    class_vector = model.generate_class_vector(
        [207], batch_size=1
