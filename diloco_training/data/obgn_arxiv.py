...
def get_ogbn_arxiv(world_size, local_rank, per_device_train_batch_size, split="train"):
    ...
    dataset = PygNodePropPredDataset(name='ogbn-arxiv', transform=ToUndirected(), root='/workspace/datasets/obgn')
    data = dataset[0]
...
if __name__ == "__main__":
    ...
    for batch, i in zip(train_loader, range(3)):
        print(f"Batch {i+1}: {batch['data'].x.shape}, {batch['data'].y.shape}, {batch['data'].batch_size}")