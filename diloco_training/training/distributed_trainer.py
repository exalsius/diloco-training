...
from diloco_training.utils.metrics_logger import MetricsLogger, log_model_config, log_dataset_config
...
class DistributedTrainer:
    def __init__(self, args):
        ...
        self.is_gan = hasattr(self.model, 'generator') and hasattr(self.model, 'discriminator')
        if self.is_gan:
            self.inner_optimizer_g, self.outer_optimizer_g = self.initialize_gan_optimizers('generator')
            self.inner_optimizer_d, self.outer_optimizer_d = self.initialize_gan_optimizers('discriminator')
            self.scheduler_g = get_cosine_schedule_with_warmup(
                self.inner_optimizer_g, num_warmup_steps=args.warmup_steps, num_training_steps=args.total_steps
            )
            self.scheduler_d = get_cosine_schedule_with_warmup(
                self.inner_optimizer_d, num_warmup_steps=args.warmup_steps, num_training_steps=args.total_steps
            )
            self.params_offloaded_g = get_offloaded_param(self.outer_optimizer_g, device=self.device)
            self.params_offloaded_d = get_offloaded_param(self.outer_optimizer_d, device=self.device)
        else:
            self.scheduler = get_cosine_schedule_with_warmup(
                self.inner_optimizer, num_warmup_steps=args.warmup_steps, num_training_steps=args.total_steps
            )
    ...
    def initialize_gan_optimizers(self, component):
        ...
        inner_optimizer = torch.optim.AdamW(
            self.model.generator.parameters() if component == 'generator' else self.model.discriminator.parameters(),
            lr=self.args.lr, weight_decay=0.1, betas=(0.9, 0.95)
        )
        ...
        optimizer_config = {"params": self.model.generator.parameters() if component == 'generator' else self.model.discriminator.parameters(),
    ...
    def _train_diloco(self):
        ...
        for step, batch in enumerate(self.train_dataloader):
            ...
            if step_within_grad_acc == 0:
                ...
                if hasattr(self.metrics_logger, 'timers') and 'forward_pass' in self.metrics_logger.timers:
                    forward_time = self.metrics_logger.timers['forward_pass'][-1] if self.metrics_logger.timers['forward_pass'] else 0
                    if forward_time > 0:
                        self.metrics_logger.log_throughput_metrics(batch_size, forward_time)
    ...
    def _train_gan(self):
        ...
        for step, batch in enumerate(self.train_dataloader):
            ...
            if step_within_grad_acc == 0:
                ...
                if self.optim_method != "ddp" and self.count_inner_optimizer_steps % self.local_steps == 0:
                    self.count_inner_optimizer_steps = 0
                    self.metrics_logger.start_timer("gan_outer_sync")
                    logger.info(f"Local rank {self.local_rank} - Syncing GAN outer optimizers at step {real_step}")
                    
                    # Sync discriminator
                    main_param_d = [param for group in self.inner_optimizer_d.param_groups for param in group["params"]]
                    bytes_sent_d = update_outer_optimizer(
                        self.params_offloaded_d, main_param_d, self.optim_method, self.world_size,
                        self.outer_optimizer_d, self.local_steps, device=self.device, 
                        quantization=self.quantization, metrics_logger=self.metrics_logger
                    )
                    self.params_offloaded_d = get_offloaded_param(self.outer_optimizer_d, device=self.device)
                    
                    # Sync generator
                    main_param_g = [param for group in self.inner_optimizer_g.param_groups for param in group["params"]]
                    bytes_sent_g = update_outer_optimizer(
                        self.params_offloaded_g, main_param_g, self.optim_method, self.world_size,
                        self.outer_optimizer_g, self.local_steps, device=self.device, 
                        quantization=self.quantization, metrics_logger=self.metrics_logger
                    )
                    self.params_offloaded_g = get_offloaded_param(self.outer_optimizer_g, device=self.device)
                ...
                if real_step % self.checkpoint_interval == 0 and not self.heterogeneous:
                    ...
                    if self.optim_method != "ddp":
                        dist.barrier()
                    log_stats(
                        self.local_rank, real_step, self.loss_batch, self.world_size, self.batch_size,
                        self.optim_method, self.sync_count, self.total_bytes_sent, val_stats,
                        self.local_steps, self.per_device_train_batch_size, self.args,
                    )
            ...
            if self.total_steps != -1 and self.total_steps <= real_step:
                ...
                if self.optim_method != "ddp":
                    ...
                    logger.info(f"Performing final sync of GAN outer optimizers at step {real_step}")
    ...
    def _sync_outer_optimizers(self, real_step):
        ...
        self.count_inner_optimizer_steps = 0
        logger.info(f"Local rank {self.local_rank} - Syncing outer optimizer at step {real_step}")
        ...
        if self.is_gan:
            ...
            bytes_sent_d = update_outer_optimizer(
                self.params_offloaded_d, main_param_d, self.optim_method, self.world_size,
                self.outer_optimizer_d, self.local_steps, device=self.device, quantization=self.quantization
            )
            self.params_offloaded_d = get_offloaded_param(self.outer_optimizer_d, device=self.device)
            ...
            bytes_sent_g = update_outer_optimizer(
                self.params_offloaded_g, main_param_g, self.optim_method, self.world_size,
                self.outer_optimizer_g, self.local_steps, device=self.device, quantization=self.quantization
            )
            self.params_offloaded_g = get_offloaded_param(self.outer_optimizer_g, device=self.device)
            
            bytes_sent = bytes_sent_d + bytes_sent_g
        else:
            main_param = [param for group in self.inner_optimizer.param_groups for param in group["params"]]
            bytes_sent = update_outer_optimizer(
                self.params_offloaded, main_param, self.optim_method, self.world_size,
                self.outer_optimizer, self.local_steps, device=self.device, quantization=self.quantization
            )
            self.params_offloaded = get_offloaded_param(self.outer_optimizer, device=self.device)
    ...
    def save_checkpoint(self, step, real_step):
        ...
        if self.is_gan:
            ...
            if self.optim_method != "ddp":
                checkpoint["outer_optimizer_state_dict"] = self.outer_optimizer.state_dict()
    ...
    def load_checkpoint(self):
        ...
        if os.path.isfile(checkpoint_file):
            ...
            if self.is_gan:
                self.inner_optimizer_g.load_state_dict(checkpoint["inner_optimizer_g_state_dict"])
                self.inner_optimizer_d.load_state_dict(checkpoint["inner_optimizer_d_state_dict"])
                self.scheduler_g.load_state_dict(checkpoint["scheduler_g_state_dict"])
                self.scheduler_d.load_state_dict(checkpoint["scheduler_d_state_dict"])
                if self.optim_method != "ddp":
                    self.outer_optimizer_g.load_state_dict(checkpoint["outer_optimizer_g_state_dict"])
                    self.outer_optimizer_d.load_state_dict(checkpoint["outer_optimizer_d_state_dict"])
            else:
