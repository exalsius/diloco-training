...
class DistributedTrainer:
    def __init__(
        ...
        self.sum_local_steps = self.config.local_steps * self.world_size

        # Initialize model
        model_class = MODEL_REGISTRY.get(self.config.model)
        assert model_class, f"Model {self.config.model} not found"
        self.model_config, self.model = self.initialize_model(model_class)

        # Initialize dataset
        dataset_class = DATASET_REGISTRY.get(self.config.dataset)
        assert dataset_class, f"Dataset {self.config.dataset} not found"
        ...
        if self.is_gan:
            ...
            self.params_offloaded_g = get_offloaded_param(
                self.outer_optimizer_g, device=self.config.device
            )
            self.params_offloaded_d = get_offloaded_param(
                self.outer_optimizer_d, device=self.config.device
            )
        else:
            ...
            self.params_offloaded = get_offloaded_param(
                self.outer_optimizer, device=self.config.device
            )

        # Other initialization
        self.scaler = GradScaler(
            device=self.config.device, enabled=(self.config.device == "cuda")
        )
        self.gradient_accumulation_steps = (
            self.config.batch_size // self.config.per_device_train_batch_size
        )
        ...
        if self.local_rank == 0:
            ...
            log_dataset_config(
                self.config.dataset, wandb_logging=self.config.wandb_logging
            )

    def heterogeneous_profiling(self):
        """Run heterogeneous profiling to optimize batch sizes and steps."""
        if dist.is_initialized() and self.config.heterogeneous:
            (
            ...
            self.config.local_steps = local_steps
            self.config.total_steps = total_steps
            self.all_info = all_info
            self.config.heterogeneous = False
    ...
    def initialize_model(self, model_class):
        ...
        if self.config.compile_model:
            try:
                logger.info(
                    f"Compiling model with torch.compile backend={self.config.compile_backend} mode={self.config.compile_mode}"
                )
                model = torch.compile(
                    model,
        ...
        if self.config.optim_method == "ddp":
            model = torch.nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.local_rank] if self.config.device == "cuda" else None,
            )
        return config, model

    def initialize_dataset(self, dataset_class):
        if self.config.dataset == "test_squence_dataset":
    ...
    def initialize_optimizers(self):
        ...
        if self.config.optim_method == "demo":
        ...
        elif self.config.optim_method in ["sgd", "sgd_quantized"]:
            optimizer_config.update({"momentum": 0.9, "nesterov": True})
            outer_optimizer = torch.optim.SGD(**optimizer_config)
        else:
            raise ValueError(f"Unknown optimization method: {self.config.optim_method}")
    ...
    def train(self):
        ...
        logger.info(
            f"Worker {self.global_rank} will execute {self.config.total_steps} steps"
        )

        if self.is_gan:
            self._train_gan()
        elif self.config.optim_method == "ddp":
            self._train_ddp()
        else:
            self._train_diloco()
    ...
    def _train_ddp(self):
        ...
        for _, batch in enumerate(self.train_dataloader):
            ...
            batch = prepare_batch(batch, device=self.config.device)
            with autocast(
                device_type=self.config.device,
                dtype=torch.bfloat16 if self.config.device == "cuda" else torch.float16,
            ):
            ...
            if step_within_grad_acc == 0:
                ...
                if (
                    real_step % self.config.checkpoint_interval == 0
                    and not self.config.heterogeneous
                ):
                ...
                self.loss_batch = (
                    0 if self.config.total_steps > real_step else self.loss_batch
                )

            if self.config.total_steps != -1 and self.config.total_steps <= real_step:
    ...
    def _train_diloco(self):
        ...
        for _, batch in enumerate(self.train_dataloader):
            ...
            if step_within_grad_acc == 0:
                ...
                logger.info(
                    f"Global rank {self.global_rank} - Local rank {self.local_rank} - Real Step {real_step} - Loss: {self.loss_batch.item()}, current step sync interval: {self.config.local_steps}"
                )
                update_inner_optimizer(
                ...
                if (
                    self.count_inner_optimizer_steps % self.config.local_steps == 0
                    and not self.config.heterogeneous
                ):
                    self.count_inner_optimizer_steps = 0
                ...
                if (
                    self.count_outer_optimizer_steps % self.config.checkpoint_interval
                    == 0
                    and not self.config.heterogeneous
                ):
    ...
    def _train_gan(self):
        ...
        for _, batch in enumerate(self.train_dataloader):
            ...
            if step_within_grad_acc == 0:
                ...
                if (
                    self.config.optim_method != "ddp"
                    and self.count_inner_optimizer_steps % self.config.local_steps == 0
                    and not self.config.heterogeneous
                ):
                    self.count_inner_optimizer_steps = 0
    ...
    def save_checkpoint(self, step, real_step):
        ...
        checkpoint = {
            ...
            "step": step,
            "optim_method": self.config.optim_method,
            "real_step": real_step,
        ...
        checkpoint_file = f"{str(self.config.checkpoint_path).split('.')[0]}.pth_node_{self.global_rank}_rank_{self.local_rank}_optim_{self.config.optim_method}.pth"
        torch.save(checkpoint, checkpoint_file)
        logger.info(
            f"Checkpoint saved for global rank {self.global_rank} and local rank {self.local_rank}, optim method {self.config.optim_method}"
        )
