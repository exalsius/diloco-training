...
class MetricsLogger:
    ...
    def log_reduce_wait_time(self, wait_time: float):
        ...
        wandb.log({
            ...
            "comm/avg_reduce_wait_time": sum(self.communication_metrics["reduce_wait_times"]) / len(self.communication_metrics["reduce_wait_times"]),
            "rank": self.local_rank,
        })
    ...
    def log_reduce_processing_time(self, processing_time: float):
        ...
        wandb.log({
            ...
            "comm/avg_reduce_processing_time": sum(self.communication_metrics["reduce_processing_times"]) / len(self.communication_metrics["reduce_processing_times"]),
            "rank": self.local_rank,
        })
    ...
    def log_throughput_metrics(self, samples_processed: int, time_elapsed: float):
        ...
        wandb.log({
            ...
            "performance/time_per_sample": time_elapsed / samples_processed if samples_processed > 0 else 0,
            "rank": self.local_rank,
        })
    
    def log_training_metrics(self, step: int, loss: float, loss_type: str = "inner", **kwargs):
...
def log_model_config(model_config: Any, model_name: str):
    ...
    if hasattr(model_config, '__dict__'):
        config_dict = {f"model_config/{k}": v for k, v in model_config.__dict__.items() 
